...
Students receive a version of FitCheck where the dashboard has three issues — one in each frontend layer (HTML, CSS, JavaScript):

- The "Add Workout" button looks completely normal but nothing happens when clicked — there's no error in the console either. This is the first opportunity to practice the skills from Section 1.6 (When the AI Assistant Gets It Wrong). When students describe the non-responsive button, the assistant may initially suggest the problem is a missing click handler in the JavaScript — a plausible guess, but wrong. Students learn to verify by checking the actual JavaScript file (the click handler code is fine) and then inspecting the button element in DevTools, where they discover the button is missing its `id` attribute. The JavaScript uses `document.getElementById('complete-workout-btn')` to find the button and attach the handler — without the `id`, the lookup returns null and the handler is never attached. This reinforces the habit of comparing what the JavaScript expects against what the HTML actually provides
- The dashboard widgets appear in the wrong order — Total Workouts shows on the left, New Workout in the middle, and Weather on the right, instead of the correct order (New Workout, Weather, Total Workouts). Students learn that in a CSS Grid layout, the visual order of elements comes from the HTML source order, not the CSS. Inspecting the HTML in DevTools reveals the `<section>` elements are in the wrong sequence
- The "Log Out" button doesn't change appearance when hovered — other buttons on the page have visible hover effects (color changes), but hovering over "Log Out" looks identical to its default state. Using DevTools to inspect the CSS, students discover that the `.btn-outline:hover` rule exists but sets the same `border-color` and `color` values as the default `.btn-outline` rule, making the hover effect invisible. The fix is to change the hover colors to something distinct (like `var(--color-primary)`)

Students use the AI coding assistant to describe what they observe, investigate the likely causes using browser DevTools, and work with the assistant to apply fixes. Each bug maps to exactly one frontend layer — JavaScript, HTML, CSS — reinforcing the core skill of distinguishing between structure, presentation, and behavior issues.
...
- After fixing the data issues, students ask the AI assistant to perform a security review of the application. The assistant flags three problems: any logged-in user can update or delete any other user's workouts — the `authorize` middleware has been removed from the PATCH and DELETE routes, so there's no ownership check on those endpoints; passwords are being stored in the database as plain text instead of being hashed; and the OpenWeatherMap API key is hardcoded directly in the backend source code rather than stored in an environment variable — meaning it's visible to anyone with access to the repository and has been committed to the Git history, where it persists even if later removed from the file
...
| 2. Frontend | HTML/CSS/JS layers, components, state, async, accessibility, DevTools | Non-functional button (JS), wrong widget order (HTML), broken hover style (CSS) | Describing what you *see* vs. what you *expect*; distinguishing HTML/CSS/JS issues |
