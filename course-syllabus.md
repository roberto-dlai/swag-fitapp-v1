...
The application students will work with throughout this course is **FitCheck** — a fitness planning application that helps users build personalized workout routines based on weather conditions, fitness goals, and personal preferences. The app features: an **HTML/CSS/JavaScript frontend** with a dashboard showing today's recommended workout, a 7-day plan, and weather conditions; a **Node.js/Express backend** that handles business logic, user management, and workout generation; a **PostgreSQL database** (relational) storing user profiles, workout history, and exercise catalogs; a **MongoDB database** (a flexible document store, explained in Module 4) storing user-generated workout reviews, ratings, and community tips; integration with the **OpenWeatherMap API** to adjust workout recommendations based on current and forecasted conditions (e.g., suggesting indoor workouts during rain, hydration reminders during heat); **user authentication** so each person has a private profile, workout history, and preferences; **user preferences and account tiers** — users can set units (metric/imperial), fitness goals, equipment availability, and workout frequency, with a free tier that limits the number of saved workout plans; and a **CI/CD pipeline** that packages the app into containers and deploys it to a cloud environment (containers and CI/CD are covered in Module 5). Students don't need to understand all of these terms yet — each one will be taught in detail throughout the course. For now, the goal is to see that FitCheck is a realistic application with many interconnected parts.
...
Once authenticated, the user lands on their dashboard. The frontend immediately makes several requests to the backend: one to fetch the user's profile and preferences, one to get today's recommended workout, and one to load the 7-day plan. For the workout recommendation, the backend also reaches out to the OpenWeatherMap API to get the current weather and forecast for the user's location. If it's raining, the backend adjusts the recommendation to suggest an indoor workout. If it's hot, it adds a hydration reminder. All of this happens in a few seconds — the user sees a brief loading spinner, then the dashboard fills in with personalized content.

The user decides to swap today's recommended run for a strength session. They click "customize," select their preferred workout type, and hit save. The frontend sends this change to the backend, which validates it (is this a real workout type? does the user's account tier allow custom plans?), updates the PostgreSQL database, and confirms the change. The dashboard updates immediately to reflect the new plan.

After completing the workout, the user logs it and writes a short review: "Great leg day, the squat progression felt right." The frontend sends the review to the backend, which stores it in MongoDB — the document database suited for flexible, text-heavy content like reviews and community tips. Other users browsing community content will see this review appear in their feed.

Later, a developer pushes a small update — a new badge feature for workout streaks. The CI/CD pipeline picks up the change, runs the automated tests to make sure nothing is broken, builds the application into a container, and deploys it to the production server. The user refreshes the page the next morning and sees their new streak badge, with no idea that anything changed behind the scenes.
...
Behind it sits the **backend**, a server that does the thinking. When a user logs in, requests a workout plan, or saves a preference, the frontend sends that request to the backend, which decides what to do with it. The backend is where the rules live: who's allowed to do what, how a workout gets generated, what happens when something goes wrong.

The backend doesn't remember anything on its own — that's what **databases** are for. FitCheck uses two kinds: a relational database (PostgreSQL) that stores structured data like user profiles and workout history in neat rows and columns, and a document database (MongoDB) that stores flexible, varied data like community reviews and tips. Together they give the application its memory.

Not everything the app needs lives inside it. The weather data that drives workout recommendations comes from an **external API** — a service run by someone else (OpenWeatherMap) that FitCheck talks to over the internet. The app also has its own **internal API**, a set of endpoints that let the frontend and backend communicate with each other. APIs are the bridges between systems, and understanding how they work is essential to diagnosing problems that cross those boundaries.
...
### The Bug: "My Workout Plan Looks Broken"

Students receive a version of FitCheck where the dashboard has visual and behavioral issues:

- The 7-day workout plan cards overlap and become unreadable when the browser window is resized to a narrower width
...
- The "Start Workout" button appears clickable but nothing happens when pressed — and this is the first opportunity to practice the skills from Section 1.6 (When the AI Assistant Gets It Wrong). When students describe the non-responsive button, the assistant may initially suggest the problem is a missing click handler in the JavaScript — a plausible guess, but wrong. Students learn to verify by asking the assistant to check the actual file, at which point the real cause emerges (the button is styled to look interactive but is actually a non-clickable element). This reinforces the habit of pushing back when a suggestion doesn't match the evidence
...
HTTP methods map to intentions. **GET** means "give me data" (fetch today's workout). **POST** means "create something new" (log a completed workout). **PUT** means "replace the entire resource" (send the user's complete updated profile to overwrite the existing one). **PATCH** means "update part of this" (change just the user's fitness goal without resending their entire profile). **DELETE** means "remove this" (delete a saved routine). The distinction between PUT and PATCH matters: PUT replaces *everything*, so you must send all fields even if only one changed, while PATCH updates only the fields you include — modern APIs typically prefer PATCH for partial updates. Using the wrong method means the server may not understand the request or may handle it differently than intended.
...
Before moving into Part B, let's trace a single request through all the concepts you've just learned. A user opens FitCheck and clicks "Generate Workout." The frontend sends a **GET** request to the endpoint `/api/workouts/today?unit=metric` with the user's authentication token in the **headers**. The backend receives this request, checks the token (authentication), reads the user's profile from the PostgreSQL database, calls the OpenWeatherMap **external API** to get the current weather, and applies business logic to generate a personalized plan. The backend sends back a **200 OK** response with a **JSON** body containing the workout data — exercises, durations, and a weather note. The frontend takes that JSON and renders it as the dashboard the user sees. If any link in this chain breaks — wrong endpoint, missing parameter, failed external API call, bad JSON — the user sees an error or wrong data. The rest of this module teaches you how to identify *where* in this chain something went wrong.
...
**Business logic** is the set of rules that make FitCheck more than just a database viewer. Rules like: "Don't recommend outdoor running when it's over 100°F," "Users on the free tier can only save 3 workout plans," "A rest day should be scheduled after 3 consecutive high-intensity days." **Validation** ensures incoming data is sensible before processing it: is the date in the right format? Is the workout duration a positive number? Is the fitness goal one of the accepted values? Input validation at the API boundary is also a security concern — checking that data is the right type, length, and format before processing it prevents many attacks. Bugs in business logic or validation cause the app to make wrong decisions or accept garbage data.
...
### The Bug: "The Weather Is Wrong and the Workouts Make No Sense"
...
- The weather widget on the dashboard always shows the temperature in Fahrenheit regardless of the user's preference setting. Open the browser's Network tab, find the `/api/weather` request, and describe to the AI assistant what parameter is missing. This is the most straightforward bug in the scenario — you'll spot the issue in the API request and understand the fix before tackling the more complex problems below
- Next, students log in as two different users — one whose profile is set to "beginner, upper body focus" and another set to "advanced, full body" — and notice that both receive the exact same generic workout plan. The personalization clearly isn't working, because two users with very different profiles should get very different recommendations. Students describe this symptom to the AI assistant ("two users with different fitness levels and goals are getting identical workout plans"), and the assistant traces the problem to the middleware pipeline: the middleware that loads user preferences runs *after* the workout generator instead of *before* it, so the generator never sees the user's profile when building the plan
- When the weather API returns a temperature above 95°F, the app should recommend indoor workouts, but instead it crashes with an unhandled error — producing a stack trace that students must share with the AI assistant to diagnose the broken temperature threshold comparison. This same lack of error handling means that when the OpenWeatherMap API is temporarily unreachable (due to network issues or brief outages — this particular outage is caused by network issues, not by exceeding the API's rate limit), the entire weather section shows "Service Unavailable" instead of falling back gracefully. Students learn that robust error handling isn't optional — it's what keeps one failure from cascading into a broken user experience

Students use the browser's Network tab and the AI assistant to trace the request-response flow through both the API layer and the backend logic, identifying where communication fails and where business rules break. Before applying any suggestion, check: does the assistant's explanation match what you observed in the Network tab? If not, push back — the evidence you gathered is more reliable than a plausible guess.
...
- Users can create new workout entries, but when they view their workout history, the "calories burned" field always shows as 0 — even though they entered values during logging. Investigation reveals a missing database migration that was supposed to add the `calories_burned` column
- A recently added "workout notes" feature crashes when any user tries to use it, displaying a database error — the schema doesn't match what the code expects
- After fixing the data issues, students ask the AI assistant to perform a security review of the application. The assistant flags three problems: any logged-in user can view any other user's workout history by simply changing the user ID in the URL — there's no authorization check on the workout history endpoint; passwords are being stored in the database as plain text instead of being hashed; and the OpenWeatherMap API key is hardcoded directly in the backend source code rather than stored in an environment variable — meaning it's visible to anyone with access to the repository and has been committed to the Git history, where it persists even if later removed from the file
...
Regardless of the testing level, every test follows the same three-step structure. **Arrange** sets up the conditions — create a test user who weighs 70 kilograms, set the weather to 98°F, load the outdoor exercise catalog. **Act** performs the action being tested — call the workout generator, submit the login form, request the calorie calculation. **Assert** checks the result — the workout should contain only indoor exercises, the login should succeed, the calorie count should be a positive number. This pattern gives students a concrete framework for describing tests to an AI assistant: "Set up a user with these characteristics, perform this action, and verify that the result is this." Understanding arrange/act/assert also helps students read and interpret test code that the assistant generates, even without writing it themselves.

#### 5.4 The Testing Pyramid
Not all tests are created equal. The **testing pyramid** describes three layers. **Unit tests** (the base, most numerous) verify individual pieces of logic in isolation — does the temperature conversion function return the right value? Does the calorie calculator produce correct results? **Integration tests** (the middle) verify that components work together — for example, when the user's profile says "beginner, indoor only" and the weather API returns "98°F," does the workout generator correctly combine both pieces and produce an indoor beginner plan? Integration tests are the bridge between testing isolated logic and testing the full user experience. **End-to-end tests** (the top, fewest) simulate a real user's full journey — can a user log in, view their dashboard, generate a workout, and log it as complete? This course focuses on unit tests and end-to-end tests in dedicated practice sections below, but when describing tests to an AI assistant, you can reference integration tests as: "Set up component A with this data, feed it into component B, and verify the combined result."
...
A **unit test** isolates a single piece of logic and verifies it works correctly. For FitCheck, unit tests might cover: does the calorie calculator return the correct value for a 70kg user cycling for 30 minutes? Does the temperature converter correctly translate 95°F to 35°C? Does the workout difficulty scoring function rate a 5km run as "moderate" for an intermediate user? Unit tests are fast (milliseconds each), numerous (dozens or hundreds), and narrowly focused. When a unit test fails, it points directly to the broken logic. Students learn to describe unit tests by specifying a single input, a single action, and a single expected output — and to ask the AI assistant to generate tests covering normal cases, edge cases, and error cases for a given function.

#### 5.8 End-to-End Tests in Practice
An **end-to-end (E2E) test** simulates a real user interacting with the full application — browser, frontend, backend, database, and external services all working together. Tools like Playwright or Cypress automate a browser to click buttons, fill forms, navigate pages, and verify what appears on screen. For FitCheck, an E2E test might: open the login page, enter credentials, verify the dashboard loads, check that today's weather is displayed, click "Generate Workout," verify that a workout plan appears with the correct number of exercises, and confirm the plan reflects indoor activities on a rainy day. E2E tests are slow (seconds to minutes each), few in number, and brittle (they can fail because of timing or layout changes), but they catch problems that no other test level can — the full chain of systems working together. Students learn to describe E2E tests as user stories: "A user who does X should see Y."
...
Tests don't run by themselves — they need a **test runner**, a tool that discovers test files, executes them, and reports results. In the JavaScript/Node.js world (FitCheck's stack), common test runners include Jest and Vitest. Test files are typically organized alongside the code they test or in a dedicated `tests/` directory, following naming conventions like `calorieCalculator.test.js`. A **test suite** is a group of related tests — all the tests for the calorie calculator, or all the tests for the user authentication flow. Students learn to ask the AI assistant to run specific test suites ("run the workout generator tests") and to interpret the structured output: which suite ran, how many tests passed, which failed, and why.
...
- A well-intentioned fix to the calorie calculation logic now returns `NaN` (Not a Number) when the user's weight is entered in kilograms instead of pounds — a unit conversion edge case that wasn't considered, and there were no tests to catch it
- The outdoor/indoor workout threshold was changed from 95°F to 35°C as part of a Celsius support update, but the weather API still returns Fahrenheit, so the threshold comparison is broken — the code checks "if the temperature is above 35, recommend indoor workouts," and since virtually all normal Fahrenheit temperatures are above 35, the condition is always true. The app now recommends indoor workouts in every condition — even on a pleasant 72°F day — making the outdoor workout feature completely broken. Students must recognize the unit mismatch and describe it to the AI assistant
...
| 2. Frontend | HTML/CSS/JS layers, components, state, async, accessibility, DevTools | Visual and interactive UI breakdowns | Describing what you *see* vs. what you *expect* |
| 3. Backend & APIs | Request-response, methods, status codes, JSON, MVC, separation of concerns, routes, middleware, business logic, stack traces | API miscommunication and broken backend logic | Tracing request flow through the middleware pipeline, using the Network tab and stack traces to pinpoint failures |
| 4. Databases & Security | SQL vs. NoSQL, schemas, CRUD, migrations, auth, hashing, OWASP, secrets management, endpoint security | Data corruption, unauthorized access, and exposed secrets | Describing data flow, persistence expectations, and articulating threat models |
| 5. Testing & Deployment | Testing pyramid, arrange/act/assert, unit tests, integration tests, E2E tests, TDD, environments, containers, CI/CD, DNS, caching, monitoring | Regressions and configuration/infrastructure failures | Specifying expected behavior precisely and reading logs to describe environment differences |
